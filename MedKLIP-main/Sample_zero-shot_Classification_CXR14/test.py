...
def test(args,config):
    ...
    disease_book = [json_book[i] + 'This disease often happens in female patients.' for i in json_book]
    # print(disease_book)
    ...
    for i, sample in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            # print('1',pred_class.shape) #(64,75,2) #先映射到大的列表上
            pred_class = F.softmax(pred_class.reshape(-1,2),dim=1).reshape(-1,len(original_class),2)
            # print(pred_class.shape) (64,75,2)
            pred_class = pred_class[:,MIMIC_mapping,1]
            # print(pred_class.shape) (64,14)
            pred = torch.cat((pred, pred_class), 0)
            
    torch.save(pred,'pred_zeroshot-biased.pt')
    torch.save(gt, 'gt-biased.pt')
    ...
    for i in range(len(target_class)):   
        ...
        # torch.save(pred_np>max_f1_thresh?1:0, '/root/lily_wu/MedKLIP-main/Sample_zero-shot_Classification_CXR14/results/pred'+target_class[i]+'.pt') 
        # torch.save(gt_np, '/root/lily_wu/MedKLIP-main/Sample_zero-shot_Classification_CXR14/results/gt'+target_class[i]+'.pt') 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='/root/lily_wu/MedKLIP-main/Sample_zero-shot_Classification_CXR14/configs/MedKLIP_config.yaml')
    parser.add_argument('--model_path', default='/root/lily_wu/MedKLIP-main/checkpoints/checkpoint_final.pth')
