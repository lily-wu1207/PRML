...
class Chestxray14_Dataset(Dataset):
    def __init__(self, csv_path,is_train = True):
        ...
        self.img_path_list = np.asarray(data_info.iloc[:,0])
        self.class_list = np.asarray(data_info.iloc[:,11:])
        self.gender = np.asarray(data_info.iloc[:,5])
        self.age = np.asarray(data_info.iloc[:,4])
    ...
    def __getitem__(self, index):
        img_path = "/root/lily_wu/datasets/images/" + self.img_path_list[index]
