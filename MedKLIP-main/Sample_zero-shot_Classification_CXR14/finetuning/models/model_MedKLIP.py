...
from adapters import ConfigUnion, PrefixTuningConfig, ParBnConfig,BnConfig
...
class MedKLIP(nn.Module):
    ...
    def __init__(self, config, disease_book):
        ...
        ''' book embedding'''
        with torch.no_grad():
            bert_model = self._get_bert_basemodel("/root/lily_wu/MedKLIP-main/Sample_zero-shot_Classification_CXR14/cache",freeze_layers = None).to(disease_book['input_ids'].device)
            # 添加adapter
            # adapter_config = ConfigUnion(
            #     PrefixTuningConfig(prefix_length=20),
            #     ParBnConfig(reduction_factor=4),
            # )
            # bert_model.add_adapter("my_adapter", config=adapter_config, set_active=True)
            #### test1
        ...
        # adapter_linear1 = nn.Linear(self.d_model, 96)
        # adapter_dropout = nn.Dropout(0.3)
        # adapter_linear2 = nn.Linear(96, self.d_model)
        ...
        # self.decoder.add_adapter("bottleneck_adapter", config=config)
        # Learnable Queries
    ...
    def forward(self, images):
        ...
        x= self.classifier(out) #B query Atributes 14,64,2
        x = x.permute(1,0,2) # 64,14,2
