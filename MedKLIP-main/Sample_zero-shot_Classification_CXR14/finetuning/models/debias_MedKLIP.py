...
class MedKLIP(nn.Module):
    ...
    def __init__(self, config, disease_book):
        ...
        decoder_layer.append(TransformerDecoderLayer(self.d_model, config['H'] , 1024,
                                        0.1, 'relu',normalize_before=True))
        adapter1 = nn.Linear(num_ftrs, num_ftrs)
    ...
    def forward(self, images):
        ...
        x= self.classifier(out) #B query Atributes 75,64,2
        x = x.permute(1,2,0) # 64,2,75
        x = self.finetune(x) # 64,2,14
        x = x.permute(0,2,1)
